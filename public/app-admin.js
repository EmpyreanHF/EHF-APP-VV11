// =====================================================
        ...
        // =====================================================
        // CLOUDINARY — app-dom.js owns the canonical
        // window.uploadToCloudinary definition (hardcoded cloud
        // name + Empyrean_preset, 90 s timeout, proper reject on
        // failure).  app-admin.js must NOT overwrite it because
        // app-admin loads AFTER app-dom.js and would replace the
        // working function with an older version that resolves
        // null/blob on failure, breaking Reels/News/Profile saves.
        //
        // We only define it here as a safety net in case app-dom.js
        // somehow didn't run (e.g. script load error).
        // =====================================================
        if (typeof window.uploadToCloudinary !== 'function') {
            // Fallback definition — mirrors app-dom.js behaviour.
            // Hardcoded credentials so uploads never depend on /api/config timing.
            const _FALLBACK_CLOUD  = 'dxcthrgsp';
            const _FALLBACK_PRESET = 'Empyrean_preset';
            const _FALLBACK_URL    = 'https://api.cloudinary.com/v1_1/' + _FALLBACK_CLOUD + '/auto/upload';
            window.uploadToCloudinary = async function uploadToCloudinary(file, onProgress) {
                ...
                return new Promise((resolve, reject) => {
                    const timeoutId = setTimeout(() => {
                        xhr.abort();
                        reject(new Error('Upload timed out after 90 s. Check your connection and try again.'));
                    }, 90000);
                    const fd = new FormData();
                    fd.append('file', file);
                    fd.append('upload_preset', _FALLBACK_PRESET);
                    fd.append('tags', 'empyrean_app');
                    const xhr = new XMLHttpRequest();
                    xhr.open('POST', _FALLBACK_URL, true);
                    ...
                    xhr.onload = () => {
                        ...
                        if (xhr.status === 200) {
                            try {
                                ...
                                if (!res.secure_url) return reject(new Error('Cloudinary returned 200 but no secure_url'));
                            ...
                            console.error('[Cloudinary] ❌ Upload failed (fallback fn):', msg, '— preset:', _FALLBACK_PRESET);
                            reject(new Error('Cloudinary upload failed: ' + msg));
                        }
                    };
                    xhr.onerror   = () => { clearTimeout(timeoutId); reject(new Error('Network error — could not reach Cloudinary')); };
                    xhr.ontimeout = () => { clearTimeout(timeoutId); reject(new Error('XHR timeout')); };
                    xhr.send(fd);
                });
            };
            console.warn('[Empyrean] uploadToCloudinary safety-net activated — app-dom.js may not have loaded.');
        }
        ...
        async function uploadMediaFilesToCloudinary(files, onProgress) {
            ...
            const uploads = Array.from(files).map(async (file, idx) => {
                ...
                // Always call window.uploadToCloudinary (not local alias) so we
                // always use the latest/best version (app-dom.js's implementation).
